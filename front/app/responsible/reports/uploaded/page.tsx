...
const ResponsibleUploadedReportsPage = () => {
  ...
  const fetchReports = async (page: number, search: string) => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/pReports/responsible/loaded`,
        {
          params: {
            page: page,
            search: search,
  ...
  const addFilesToAttachments = (files: File[]) => {
    if (
      ...
      files.some((file) =>
        selectedReport?.filled_reports[0].attachments.some(
          ...
          <Table.Tr key={pubReport._id}>
            ...
            <Table.Td>
              <Center>
                <Badge
                  bg={
                    pubReport.filled_reports[0].status === "Rechazado"
                      ? theme.colors.red[9]
          ...
          <Pill
            ...
            onClick={() => {
              if (typeof window !== "undefined") {
                window.open(
                  selectedReport?.filled_reports[0].report_file.view_link
                );
              }
            }}
          >
